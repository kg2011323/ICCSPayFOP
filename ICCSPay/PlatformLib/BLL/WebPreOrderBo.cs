...
namespace PlatformLib.BLL
{
    ...
    public class WebPreOrderBo
    {
        ...
        /// 微信用户OpenID获取查询
        /// </summary>
        /// <param name="userOpenIDQueryRequestVo"></param>
        /// <returns></returns>
        public WXUserOpenIDRespVo WXUserOpenIDOrderQuery(WXUserOpenIDReqVo uoReqVo, ConcurrentDictionary<string, string> S1_006Dict)
        {
            WXUserOpenIDRespVo result = new WXUserOpenIDRespVo();
            result.respCodeMemo = String.Empty;
            result.RespondCode = DeviceCommRespondCode.RC0005;
            try
            {
                if (null != uoReqVo)
                {
                    ...
                    string inputRandomFact = uoReqVo.randomFact;

                    string inputDeviceId = uoReqVo.DeviceId;

                    _log.Debug("Request data: " + uoReqVo.ReqSysDate + " Request operateCode: " + uoReqVo.OperationCode + " Request cityCode: " + uoReqVo.CityCode + " Request channelType: " + uoReqVo.ChannelType + " Request DeviceId: " + uoReqVo.DeviceId + " Request RandomFact: " + uoReqVo.randomFact);
                    ...
                    if (!String.IsNullOrEmpty(inputRandomFact))
                    {
                        using (MobilePayDBEntities dbContext = new MobilePayDBEntities())
                        {
                                ...
                                && qrcWebOrder.RandomFact.Equals(inputRandomFact))
                            {
                                if (qrcWebOrder.DeviceId == inputDeviceId)
                                {
                                    if (qrcWebOrder.OrderStatus == "3" && qrcWebOrder.Step == "11" || qrcWebOrder.Step == "17")//重置查询条件
                                    {
                                        result.RespondCode = DeviceCommRespondCode.RC0000;
                                        //添加获取用户OpenID逻辑
                                        result.userAccount = GetRandomString(28, true, true, true, false, "");
                                        //result.userAccount = "123456";
                                        _log.Info("已成功获取到微信用户ID！");
                                    }
                                    else
                                    {
                                        result.RespondCode = DeviceCommRespondCode.RC0004;
