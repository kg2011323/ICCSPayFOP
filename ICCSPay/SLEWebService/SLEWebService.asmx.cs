...
namespace SLEWebService
{
    ...
    public class SLEWebService : System.Web.Services.WebService
    {
        ...
        /// 用户OPENID查询接口
        /// </summary>
        /// <param name="userOpenIDQueryReq"></param>
        /// <returns></returns>
        [WebMethod]
        public UserOpenIDQueryResp S1_021(UserOpenIDQueryReq userOpenIDQueryReq)
        {
            Stopwatch sw = new Stopwatch();
            sw.Start();
            string strReqLog = "", strRespLog = "";
            try
            {
                if (null != userOpenIDQueryReq)
                {
                    string strTVReq = "reqSysDate:" + userOpenIDQueryReq.reqSysDate + ",deviceId:" + userOpenIDQueryReq.deviceId + ",channelType:" + userOpenIDQueryReq.channelType + ",orderNo:" + userOpenIDQueryReq.randomFact;
                    strReqLog = String.Format("Req:{0}:{1}", RequestIP, strTVReq);
                }
            }
            catch (Exception ex)
            ...
            UserOpenIDQueryResp userOpenIDQueryResponce = new UserOpenIDQueryResp();
            userOpenIDQueryResponce.respCode = _respondCodeDefault;
            WebPreOrderBo bo = new WebPreOrderBo();
            try
            {
                if (null == userOpenIDQueryReq)
                {
                    userOpenIDQueryResponce.respCode = _respondCodeParaErr;
                }
                else
                {
                    bool isDeviceIdValid = true;
                    ...
                    if (Constants.IsCheckDeviceId)
                    {
                        if (!DeviceHelper.Instance.IsValidDeviceId(userOpenIDQueryReq.deviceId))
                        {
                            isDeviceIdValid = false;
                            userOpenIDQueryResponce.respCode = _respondCodeDeviceInvalid;
                        }
                    }

                    if (isDeviceIdValid)
                    {
                        userOpenIDQueryResponce.respCode = _respondCodeOrderNoInvalid;

                        if (!String.IsNullOrEmpty(userOpenIDQueryReq.randomFact))
                        {
                            #region 查询微信用户OpenID
                            WXUserOpenIDReqVo uoReqVo = new WXUserOpenIDReqVo();
                            uoReqVo.ReqSysDateString = userOpenIDQueryReq.reqSysDate;
                            uoReqVo.OperationCode = userOpenIDQueryReq.operationCode;
                            uoReqVo.CityCode = userOpenIDQueryReq.cityCode;
                            uoReqVo.DeviceId = userOpenIDQueryReq.deviceId;
                            uoReqVo.ChannelType = userOpenIDQueryReq.channelType;
                            uoReqVo.expandAttribute = userOpenIDQueryReq.expandAttribute;
                            uoReqVo.randomFact = userOpenIDQueryReq.randomFact;

                            WXUserOpenIDRespVo uoRespVo = bo.WXUserOpenIDOrderQuery(uoReqVo, Global.eh.S1Dict);  //在预购逻辑类增加相应的用户获取方法
                            userOpenIDQueryResponce.respCode = uoRespVo.RespondCodeString;
                            userOpenIDQueryResponce.respCodeMemo = uoRespVo.respCodeMemo;
                            userOpenIDQueryResponce.userAccount = uoRespVo.userAccount;//uoRespVo.userAccount;
                            #endregion 查询微信用户OpenID
                        }
                    }
                }
            }
            ...
            try
            {
                string strOEBResp = "respCode:" + userOpenIDQueryResponce.respCode + ",userAccount:" + userOpenIDQueryResponce.userAccount;
                strRespLog = String.Format("Resp:{{{0}}}", strOEBResp);
            }
            catch (Exception ex)
            {
            ...
            sw.Stop();
            TimeSpan ts = sw.Elapsed;
            if (userOpenIDQueryResponce.respCode == "0000")
            {
                _log.Info(strRespLog);
            }
            else
            ...
            return userOpenIDQueryResponce;
        }
